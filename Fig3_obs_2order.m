...
% % 创建第一个坐标系
% f1=axes('Position', [left, bot, width, height]); % 左下角坐标 (0.1, 0.7)，宽度和高度各为 0.25
% pcolor(x2,z2,Tux);shading interp;colorbar;hold on;
% contour(x2,z2,rho2,[22.5:pycnal:24],'linewi',.5,'linestyle','-','color',colorcon);
% plot(mld_CDx(1,:),mld,'color','r','linestyle','--','LineWidth',1.5)
% % colortable=textread('Turner.txt');
% colormap(f1,flipud(cmap));
% c=colorbar;
% set(c,'ytick',[-pi/2 -pi/4 pi/4 pi/2],'yticklabel',{'-pi/2','-pi/4','pi/4','pi/2'})
% caxis([-pi/2 pi/2]);
% ylabel('depth [m]');
% text(1.5,-50,'Tux')
% text(17,5,'Transect CD','FontWeight','b')
...
% 
% f2=axes('Position', [left, bot-zpos*1, width, height]); 
% pcolor(x2,z2,Tuz);shading interp;colorbar;hold on;
% contour(x2,z2,rho2,[22.5:pycnal:24],'linewi',.5,'linestyle','-','color',colorcon);
% plot(mld_CDx(1,:),mld,'color','r','linestyle','--','LineWidth',1.5)
% colormap(f2,flipud(cmap));
% c=colorbar;
% set(c,'ytick',[-pi/2 -pi/4 pi/4 pi/2],'yticklabel',{'-pi/2','-pi/4','pi/4','pi/2'})
% caxis([-pi/2 pi/2]);
% ylabel('depth [m]');
